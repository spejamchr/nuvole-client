...
export default class CurrentUserStore {
  ...
  misfiredState = (action: string): void =>
    logMisfiredState('CurrentUserStore', action, this.state);
  ...
  ready = (resource: CurrentUserResource) => {
    switch (this.state.kind) {
      case 'loading':
  ...
  get resource(): Result<Exclude<State, Ready>, CurrentUserResource> {
    switch (this.state.kind) {
      case 'waiting':
      case 'loading':
      case 'loading-error':
      ...
      case 'ready':
        return ok(this.state.currentUser);
    }
  }
}
